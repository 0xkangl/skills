...
> **After a module is created**: ensure it has its own `CLAUDE.md` with Role, Mandatory Specs (relative links to {{PROJECT}}-spec-center), Key Responsibilities, Tech Stack, Build & Test, and Deployment sections.
...
| Module deployment / ops config | Module's `CLAUDE.md` §Deployment（平台、配置文件、发版/回滚命令）+ `docs/` |
