...
首次上线前按实际使用的资源备齐。缺少必要资料时暂停依赖它的部署步骤，继续已授权的本地构建、只读检查和方案准备；未使用的数据库、自定义域名等不构成阻塞。

### 操作范围与授权

- 对任务相关的服务器和平台执行只读检查时，核对账号、项目和环境，避免输出密钥或无关项目数据。
- 状态变更前说明具体操作、目标资源、配置或数据影响、恢复方式及对其他项目的影响。已有明确授权覆盖时按依赖顺序执行并逐步验证，不为每一步重复确认；平台或 CI 的权限门禁仍须满足。
- 准备发布不自动授权创建付费资源、修改共享 DNS、删除数据或发布生产版本；这些行为是否属于原任务，需要结合用户明确范围判断。
- 出现新重大风险或验证失败时，暂停后续变更并说明；仅执行原授权和适用预案覆盖的恢复操作，不因“必须回滚”擅自扩大数据或资源变更范围。
...
6. **验证**（**退出码 0 不算生效证据**）：
   ...
   - 任一项不过 → 停止后续发布，按已授权且适用的预案恢复；回滚不安全或超出授权时保留现场并报告所需决策，不盲目回滚数据。
