...
- 模板在 `templates/`，用 `{{PROJECT}}` 占位。模板以 `AGENTS.md` 为正文第一文件，`CLAUDE.md` 只是一行 `@AGENTS.md` 指针。模板 `spec-center/AGENTS.md` 的脚本维护区用锚点注释（`<!-- MODULE_MAP_START/END -->`、`<!-- REPO_TREE_START/END -->`）标出，不要手工编辑锚点之间的内容。
