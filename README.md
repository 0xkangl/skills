...
- `api-audit` 设 `disable-model-invocation: true`，仅手动 `@api-audit` 触发，可对任意 HTTP 服务独立运行。它与 `codebase-audit` **正交不合并**：后者审通用代码质量七维度，前者只审**接口逻辑**的完备性/自洽性与业务流程闭环，复用同一套「main agent 只 scope+编排 → 并行审计 → 对抗式 verify 分离 → 汇总」范式但聚焦点与产物（两份报告）不同。
